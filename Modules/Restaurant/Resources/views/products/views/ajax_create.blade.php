<div class="text-center">
    <h2>
        {{ 'Nueva '.$dataType->getTranslatedAttribute('display_name_singular') }}
    </h2>
</div>
<div class="row">
    <div class="col-md-12">
        <form  action="{{ route('myproducts_ajax_store', 'Category') }}" id="myform_modal" method="POST" enctype="multipart/form-data">
        {{ csrf_field() }}
            @foreach($dataRows as $row)
                ...
                <div class="form-group @if($row->type == 'hidden') hidden @endif col-md-{{ $display_options->width ?? 12 }} {{ $errors->has($row->field) ? 'has-error' : '' }}" @if(isset($display_options->id)){{ "id=$display_options->id" }}@endif>
                    ...
                    @if ($row->add)
                        @switch($row->type)
                        @case('relationship')
                            <label class="" for="{{ $row->field }}">{{ $row->display_name }}</label>
                        ...
                        @case('select_dropdown')
                            ...
                            @break
                        @case('text')
                            <label calss="" for="{{ $row->field }}">{{ $row->display_name }}</label>
                            @if(isset($row->details->tooltip))
                                <span class="voyager-question"
                                aria-hidden="true"
                                data-toggle="tooltip"
