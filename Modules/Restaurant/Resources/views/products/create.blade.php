...
@section('page_header')
     <div class="container-fluid">
        ...
        <ul class="dropdown-menu">
            <li><a href="javascript:;" id="save">Guardar Producto</a></li>
            <li><a href="{{ route('myproducts.create') }}">Guardar y Continuar</a></li>
            <li role="separator" class="divider"></li>
            <li><a href="javascript:;" id="new_category">Nueva Categoria</a></li>
            <li><a href="javascript:;" id="new_subcategory">Nueva Sub Categoria</a></li>
            <li role="separator" class="divider"></li>
    ...
    <div class="row">
        <div class="col-md-12">
             <div class="panel panel-primary panel-bordered">
                <form class="myform" role="form" action="{{ route('myproducts.store') }}" id="myform" method="POST" enctype="multipart/form-data">
                {{ csrf_field() }}
...
@section('javascript')
    ...
    <script>    
        $('document').ready(function () {
            ...
            let sub_category_id = '{{ route('myproducts_ajax_index', [':sub_category_id', 'SubCategory']) }}';
            sub_category_id = sub_category_id.replace(':sub_category_id', $("#category_id :selected").val());
            myselect2(sub_category_id, '#sub_category_id');
        ...
        $('#category_id').change(function(){
            let urli ="{{ route('myproducts_ajax_index', [':sub_category_id', 'SubCategory']) }}";
            urli = urli.replace(':sub_category_id', $(this).val());
            let comp = "#sub_category_id";
            myselect2(urli, comp);
        });
        ...
        $('#new_category').click(function(){
            ...
            $.ajax({
                type: "get",
                url: "{{ route('myproducts_ajax_create', 'categories') }}",
                success: function (response) {
                    $('#title_modal').html('<h4 class="modal-title"><i class="voyager-edit"></i> Nueva Categoria</h4>');
        ...
        $('#new_subcategory').click(function(){
            ...
            $.ajax({
                type: "get",
                url: "{{ route('myproducts_ajax_create', 'sub_categories') }}",
                success: function (response) {
                    $('#title_modal').html('<h4 class="modal-title"><i class="voyager-edit"></i> Nueva Categoria</h4>');
                    $('#modal_body').html(response);
                    $('#default_modal').modal('show');
        ...
        //funcines -------------------------------
        //--------------------------------------
        function myselect2(urli, comp){
            $.ajax({
                type: "get",
                url: urli,
                success: function (response) {
                    $(comp).select2('destroy');
                    let count=0;
                    response.forEach( function( item ) {
                        if(count==0)
                        {
                            response += `<option disabled selectd>-- Seleciona un dato --</option>`;
                            count++;
                        }
                    ...
                    const Toast = Swal.mixin({
                        ...
                        Toast.fire({
                        icon: 'info',
                        title: 'Categorias y Sub C. Actualizados'
                    })
                }
            });
        }
